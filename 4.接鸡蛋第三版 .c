...
struct location
{
	...
	*/
	int	hand_x ,	egg_x,	 egg_y ,	  d_hand_x,	  d_egg_x,	d_egg_y;
...
int main(void)
{
	...
	struct location play = {5,(int)(10 * rand() / (RAND_MAX + 1)) ,0,0,0,0};
...
void Move(struct location * p_play )							//游戏移动
{
	...
	*/ 
	...
	if(	p_play->hand_x == p_play->egg_x && 9 == p_play->egg_y)         //写得分
	{ 
	score+=10;
	gotoxy(8,0,1);
	printf("%2d",score);
	} 
	else if(p_play->egg_y == 9 && p_play->hand_x != p_play->egg_x)		//写生命
	{	
	life-=1;
	printf("\7");
	gotoxy(32,0,1);
